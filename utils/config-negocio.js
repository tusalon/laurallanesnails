...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '0a129e7a-faf0-4ec3-a8c6-aee1a551d694'; // ID de LauraLLanesNails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'LauraLLanesNails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '53833588';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'laurallago@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a LauraLLanesNails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'laurallanesnails';
};
...
console.log('✅ config-negocio.js listo para LauraLLanesNails');
